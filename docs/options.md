...
| `layerChangeAmount`           | integer | `1`     | `1` to `30000000` | Number of layers moved by one layer next/previous action. Applies to Litematica layer hotkeys and CrossPatch Box Layer hotkeys. Can also be changed in game with `layerChangeAmountModifier`. |
...
| `renderEntitiesAsTranslucent`        | boolean | `false` | -            | Renders the schematic's entities (armor stands, item frames, mobs...) translucent so they can be told apart from real world entities. Complements Litematica's `renderBlocksAsTranslucent`, which only ghosts blocks; enable both together. |
