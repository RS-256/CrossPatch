...
public class StonecutterRenderEventHandler {
    private static final StonecutterRenderEventHandler INSTANCE = new StonecutterRenderEventHandler();
    ...
    public static StonecutterRenderEventHandler instance() {
        return INSTANCE;
    }
    ...
    public void onDrawScreenPost(GuiContext ctx, Minecraft mc, int mouseX, int mouseY) {
        ...
        if (GuiUtils.getCurrentScreen() instanceof StonecutterScreen gui && StonecutterInputUtils.isRecipeViewOpen()) {
            ...
            if (recipeId >= 0) {
                ...
                ItemStack stack = this.getHoveredRecipeIngredient(mouseX, mouseY, recipes.getRecipe(recipes.getSelection()), gui);
    ...
    private void renderRecipeView(GuiContext ctx, Minecraft mc, int mouseX, int mouseY) {
        if (GuiUtils.getCurrentScreen() instanceof StonecutterScreen gui && StonecutterInputUtils.isRecipeViewOpen()) {
            ...
            StonecutterRecipePattern recipe = recipes.getRecipe(Math.max(0, this.getHoveredRecipeId(mouseX, mouseY, recipes, gui)));
            if (this.getHoveredRecipeId(mouseX, mouseY, recipes, gui) < 0) {
                recipe = recipes.getRecipe(recipes.getSelection());
            }
    ...
    private void renderHoverTooltip(GuiContext ctx, int mouseX, int mouseY, StonecutterRecipePattern recipe) {
    ...
    public int getHoveredRecipeId(int mouseX, int mouseY, StonecutterRecipeStorage recipes, AbstractContainerScreen<?> gui) {
    ...
    private void renderRecipeItems(GuiContext ctx, StonecutterRecipePattern recipe) {
        this.renderStackAt(ctx, recipe.getIngredient(), -3 * 17 + 2, 3 * this.entryHeight, false);
    }

    private ItemStack getHoveredRecipeIngredient(int mouseX, int mouseY, StonecutterRecipePattern recipe, AbstractContainerScreen<?> gui) {
        ...
        if (mouseX >= x && mouseX <= x + scaledStackDimensions &&
                mouseY >= y && mouseY <= y + scaledStackDimensions) {
